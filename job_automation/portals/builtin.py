...
class BuiltInWorker(BasePortalWorker):
    ...
    async def search_jobs(self, page: Page) -> list[RawJob]:
        ...
        for query in self.config.search_queries:
            ...
            url = f"{self.base_url}?search={quote_plus(query)}&remote=true"
            await page.goto(url, wait_until="domcontentloaded", timeout=60000)
            await page.wait_for_timeout(2000)

            for page_num in range(self.config.max_pages_per_query):
                cards = page.locator("div[data-id], .job-bubble, a[href*='/job/']")
                count = await cards.count()
                if count == 0:
                    break
                for i in range(min(count, 25)):
                    card = cards.nth(i)
                    link = card if await card.get_attribute("href") else card.locator("a").first
                ...
                next_btn = page.locator("a[rel='next'], button:has-text('Next')").first
    ...
    async def open_job(self, page: Page, raw_job: RawJob) -> JobDetail:
        ...
        company = await safe_inner_text(page, ".company-title, [data-company]")
        location = await safe_inner_text(page, ".job-location, .location")
        salary = await safe_inner_text(page, ".job-salary, .salary")
    ...
    async def extract_apply_url(self, page: Page) -> str | None:
        return await click_apply_and_get_url(
            page,
            ["a:has-text('Apply')", "a:has-text('Apply Now')", "a.apply-button", "a[href*='apply']"],
        )

    async def extract_description(self, page: Page) -> str | None:
        for selector in [".job-description", "section.description", "article", "main"]:
